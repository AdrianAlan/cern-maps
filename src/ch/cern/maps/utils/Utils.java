...
public class Utils {
	...
	public static String getNextTrains(ArrayList<Trams> tramList) {
		...
		for (Trams t : tramList) {
			...
			int toGo = (60*24*Integer.parseInt(time[0])+Integer.parseInt(time[1])*60+Integer.parseInt(time[2]))-(24*60*(nowTime.getDay()-1)+nowTime.getHours()*60+nowTime.getMinutes());
			if (toGo < 0) {
				toGo += 60*24*7;
			}
			minimumTime(minA, minB, toGo);
		}
		return "" + Math.min(minA, minB) + "min; " + Math.max(minA, minB) + "min.";
	}
	...
	public static double[] getPixel(double vertical, double horizontal) {
		...
		double picV = picHeight*(1 -((vertical-maxBottom)/(maxTop-maxBottom)));
		double picH = picWidth*((horizontal-maxLeft)/(maxRight-maxLeft));
