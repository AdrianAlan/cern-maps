...
public class JSONParser {
	...
	public JSONParser(InputStream is) {
		...
		try {
			...
			Log.e(Constants.TAG, "Error converting result to " + e.toString(), e);
		} finally {
			try {
				br.close();
			} catch (IOException e) {
		...
		try {
			...
			Log.e(Constants.TAG, "Error parsing data " + e.toString(), e);
		}
	}
	...
	public ArrayList<Trams> readSchedule(String lineNumber) {
		ArrayList<Trams> trams = new ArrayList<>();
		try {
	...
	public Building readBuildingCoordinants(String searchingFor) {
		try {
			final JSONObject theBuilding = jObj.getJSONObject(searchingFor);
			return new Building(searchingFor.toLowerCase(Locale.getDefault()),
					theBuilding.getString(Constants.JSON_TAG_BUILDINGS_LATITUDE),
					theBuilding.getString(Constants.JSON_TAG_BUILDINGS_LONGITUDE),
	...
	public Circuit readShuttle(String lineName) {
		...
		try {
			...
			Map<String, ArrayList<String>> stopList = new HashMap<>();
